...
class Simu {
    ...
    bool can_build_ore_robot(const State &s) const {
        ...
        static const v4su vtrue = v4su{0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF};
        v4su t = (s.stock >= bp.ore);
        return ::memcmp(&vtrue, &t, sizeof(t)) == 0;
    }
...
void part_1_2_rec(const std::string &filename, bool part2 = false) {
    std::list<Blueprint> blueprints;
    if (!parse_input(filename, blueprints)) {
        return;
    }
...
int main(int argc, char **argv) {
    ...
    part_1_2_rec(opt_filename);
    part_1_2_rec(opt_filename, true);
